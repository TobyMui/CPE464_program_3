...
int safeSendto(int socketNum, void *buf, int len, int flags, struct sockaddr *destAddr, int addrLen)
{
    int returnValue = 0;
    if ((returnValue = sendtoErr(socketNum, buf, (size_t) len, flags, (struct sockaddr *)destAddr, (socklen_t)addrLen)) < 0)
    {
        perror("sendtoErr: ");
        exit(-1);
    }
